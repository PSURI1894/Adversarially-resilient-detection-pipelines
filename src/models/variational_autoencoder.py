...
class VAIDS(Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            reconstruction_loss = tf.reduce_mean(
                tf.reduce_sum(tf.keras.losses.mean_squared_error(data, reconstruction), axis=-1)*self.input_dim
            )
    ...
    def predict_proba(self, X):
        ...
        prob_malicious = 1 / (1 + np.exp(- (scores - scores.mean()) / (scores.std() + 1e-6)))
