...
class PGDTrainer(BaseAdversarialTrainer):
    ...
    def generate_adversarial(self, x, y):
        ...
        noise = tf.random.uniform(
            tf.shape(x), minval=-self.epsilon, maxval=self.epsilon
        )
        x_adv = x_adv + noise
...
class TRADESTrainer(BaseAdversarialTrainer):
    ...
    def __init__(
        self, model, epsilon=0.1, alpha=0.02, iters=5, beta=6.0, mutable_features=None
    ):
        super().__init__(model, epsilon, mutable_features)
        self.alpha = alpha
        self.iters = iters
    ...
    def train_step(self, x, y, optimizer):
        ...
        for _ in range(self.iters):
            with tf.GradientTape() as tape:
                ...
                kl = probs_clean * tf.math.log(
                    probs_clean / (probs_adv + 1e-8) + 1e-8
                ) + (1 - probs_clean) * tf.math.log(
                    (1 - probs_clean) / (1 - probs_adv + 1e-8) + 1e-8
                )
        ...
        with tf.GradientTape() as tape:
            ...
            ce_loss = tf.keras.losses.binary_crossentropy(
                tf.expand_dims(tf.cast(y, tf.float32), -1),
                logits_clean_train,
                from_logits=True,
            )
            ...
            kl_train = probs_clean_train * tf.math.log(
                probs_clean_train / (probs_adv_train + 1e-8) + 1e-8
            ) + (1 - probs_clean_train) * tf.math.log(
                (1 - probs_clean_train) / (1 - probs_adv_train + 1e-8) + 1e-8
            )
